...
public class MinesweeperScreen extends Screen {
    ...
    protected void init() {
        ...
        if (gameMode == GameMode.DEFAULT) {
            ...
            if (board.w!=normalizedSetting.width()||board.h!=normalizedSetting.height()||board.mines!=normalizedSetting.mines()) {
                ...
                if (minecraft != null) {
                    minecraft.setScreen(new MinesweeperScreen(normalizedSetting, GameMode.DEFAULT));
                }
            }
        }
        ...
        if (gameMode == GameMode.DEFAULT) {
            ...
            SpriteIconButton configBtn = SpriteIconButton.builder(
                    Component.empty(),
                    (button) -> {
                        minecraft.setScreen(AutoConfigClient.getConfigScreen(Config.class, this).get());
                    },
                    true
            ...
            SpriteIconButton minigamesBtn = SpriteIconButton.builder(
                    Component.empty(),
                    (button) -> {
                        minecraft.setScreen(new DownloadMinigamesScreen());
                    },
                    true
            ).width(20).sprite(Identifier.fromNamespaceAndPath("minesweeper", "icon/minigames"), 18, 18).build();
            minigamesBtn.setPosition(width-8-20-20-5, 8);
            this.addRenderableWidget(minigamesBtn);
        }
    }
    ...
    protected GameBoard.SoundCallback makeSoundCallback() {
        return new GameBoard.SoundCallback() {
            ...
            public void onExplode(int cellX, int cellY) {
                mc.getSoundManager().play(SimpleSoundInstance.forUI(
                    SoundEvents.GENERIC_EXPLODE.value(), 0.7f, (float) MinesweeperMod.CONFIG.soundsVolume/100));
            ...
            public void onWin() {
                mc.getSoundManager().play(SimpleSoundInstance.forUI(
                    SoundEvents.FIREWORK_ROCKET_BLAST, 0.8f, (float) MinesweeperMod.CONFIG.soundsVolume/100));
            }
        };
    }
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        ...
        mc.getSoundManager().play(SimpleSoundInstance.forUI(
                SoundEvents.NOTE_BLOCK_HAT.value(), 0.20f, (float) MinesweeperMod.CONFIG.soundsVolume/100));
