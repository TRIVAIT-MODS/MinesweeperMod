...
public class MinesweeperScreen extends Screen {
    ...
    private static class Animation {
        ...
        void draw(DrawContext context, String modId) {
            ...
            context.drawTexture(RenderLayer::getGuiTextured, tex, x, y, 0, 0, width, height, width, height);
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        double scaleFactor = mc.getWindow().getScaleFactor();
        float textScale = (scaleFactor <= 2) ? 1.2f : 0.8f;
        ...
        for (int yy = 0; yy < h; yy++) {
            for (int xx = 0; xx < w; xx++) {
                ...
                float scale = 1.0f;
                boolean animActive = c.revealProgress >= 0f && c.revealProgress < 1f;
                if (animActive) {
                    float p = c.revealProgress;
                    float phase = p < 0.5f ? (p / 0.5f) : ((1f - p) / 0.5f);
                    scale = 1.0f + phase * 0.20f;
                }

                context.getMatrices().push();
                context.getMatrices().translate(x + cellSize / 2f, y + cellSize / 2f, 0f);
                ...
                if (c.revealed) {
                    if (c.mine && !won) {
                        if (c.flagged && !alive) {
                            ...
                            context.drawTexture(RenderLayer::getGuiTextured, FLAG, offsetX, offsetY, 0, 0,
                            ...
                            int offsetX = x + (cellSize - texSizeAdjust) / 2;
                            int offsetY = y + (cellSize - texSizeAdjust) / 2;
                            context.drawTexture(RenderLayer::getGuiTextured, TNT_SIDE, offsetX, offsetY, 0, 0,
                                    texSizeAdjust, texSizeAdjust, texSizeAdjust, texSizeAdjust);
                        }
                    ...
                    if (!alive && !won && !c.mine) {
                        ...
                        int offsetX = x + (cellSize - texSizeAdjust) / 2;
                        int offsetY = y + (cellSize - texSizeAdjust) / 2;
                        context.drawTexture(RenderLayer::getGuiTextured, BARRIER, offsetX, offsetY, 0, 0,
                                texSizeAdjust, texSizeAdjust, texSizeAdjust, texSizeAdjust);
                    } else {
                        Identifier FLAG = Identifier.of(MinesweeperModClient.MOD_ID, "textures/gui/flag.png");
                        int offsetX = x + (cellSize - texSizeAdjust) / 2;
        ...
        if (!animations.isEmpty()) {
            for (int i = animations.size() - 1; i >= 0; i--) {
                ...
                a.draw(context, MinesweeperModClient.MOD_ID);
