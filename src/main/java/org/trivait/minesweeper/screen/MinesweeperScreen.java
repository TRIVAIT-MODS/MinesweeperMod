...
public class MinesweeperScreen extends Screen {
    ...
    private static final Function<Identifier, RenderLayer> RENDER_LAYER = RenderLayer::getGuiTextured;
    ...
    private class SmileyButtonWidget extends ClickableWidget {
        ...
        protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            context.drawTexture(RENDER_LAYER, tex, ix, iy, 0, 0, iw, ih, iw, ih);
        }
    ...
    public MinesweeperScreen() {
        super(Text.literal("Minesweeper"));
    }
        ...
        for (int yy = 0; yy < h; yy++) {
            for (int xx = 0; xx < w; xx++) {
                ...
                if (isHighScale) {
                    ...
                    context.getMatrices().scale(p, p, 1.0f);
                ...
                if (contentScale != 1.0f) context.getMatrices().scale(contentScale, contentScale, 1f);
                context.getMatrices().translate(-cx, -cy, 0f);

                if (c.revealed) {
                    if (c.mine && !won) {
                        int ox = cx - texSizeAdjust / 2, oy = cy - texSizeAdjust / 2;
                        context.drawTexture(RENDER_LAYER, c.flagged && !alive ? TEX_FLAG : TEX_TNT_SIDE, ox, oy, 0, 0, texSizeAdjust, texSizeAdjust, texSizeAdjust, texSizeAdjust);
                    } else if (c.adjacent > 0) {
                        Text numText = ADJ_TEXT[c.adjacent];
                        int color = getAdjColor(c.adjacent);
                        context.getMatrices().push();
                        context.getMatrices().translate(cx, cy, 0f);
                        if (textScale != 1.0f) context.getMatrices().scale(textScale, textScale, 1f);
                        context.getMatrices().translate(-mc.textRenderer.getWidth(numText) / 2f, -mc.textRenderer.fontHeight / 2f, 0f);
                        context.drawText(mc.textRenderer, numText, 0, 0, color, false);
                        context.getMatrices().pop();
                    }
                } else if (c.flagged) {
                    int ox = cx - texSizeAdjust / 2, oy = cy - texSizeAdjust / 2;
                    context.drawTexture(RENDER_LAYER, !alive && !won && !c.mine ? TEX_BARRIER : TEX_FLAG, ox, oy, 0, 0, texSizeAdjust, texSizeAdjust, texSizeAdjust, texSizeAdjust);
                }
                context.getMatrices().pop();
            }
        }
