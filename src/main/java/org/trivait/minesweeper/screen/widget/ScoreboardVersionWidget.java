...
public class ScoreboardVersionWidget extends ClickableWidget {
    ...
    protected void renderWidget(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ctx.drawTexture(TEX_SCORE, getX(), getY(), 0, 0, SIZE, SIZE, SIZE, SIZE);
    ...
    private List<Text> buildTooltip() {
        ...
        lines.add(Text.translatable("scoreboard.version.script", scriptVer));
        lines.add(Text.translatable("scoreboard.version.api", apiVer));
        if (SheetsApi.isVersionMismatch()) {
            lines.add(Text.translatable("scoreboard.version.update_mod").styled(s -> s.withColor(0xFF5555)));
        }
        return lines;
    }
