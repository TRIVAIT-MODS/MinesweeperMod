...
public class LeaderboardMinesweeperScreen extends MinesweeperScreen {
    ...
    public void onClose() {
        ...
        if (saved != null) {
            Minecraft.getInstance().setScreen(new MinesweeperScreen(saved, GameMode.DEFAULT));
        } else {
            Minecraft.getInstance().setScreen(new MinesweeperScreen(new GameSettings(cfg.gridWidth, cfg.gridHeight, cfg.mines), GameMode.DEFAULT));
        }
    }
}
