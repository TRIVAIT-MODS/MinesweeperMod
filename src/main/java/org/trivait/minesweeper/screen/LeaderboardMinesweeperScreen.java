...
public class LeaderboardMinesweeperScreen extends MinesweeperScreen {
    ...
    private void drawTopCounter(DrawContext ctx) {
        ...
        ctx.getMatrices().translate(width / 2f, 4f, 0f);
        ctx.getMatrices().scale(2f, 2f, 1f);
        ctx.getMatrices().translate(-tw / 2f, 0f, 0f);
    ...
    public void close() {
        ...
        if (saved != null) {
            MinecraftClient.getInstance().setScreen(new MinesweeperScreen(saved, GameMode.DEFAULT));
        } else {
            MinecraftClient.getInstance().setScreen(new MinesweeperScreen(new GameSettings(cfg.gridWidth, cfg.gridHeight, cfg.mines), GameMode.DEFAULT));
        }
    }
}
